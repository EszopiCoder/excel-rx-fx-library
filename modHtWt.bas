...
Private Sub TestHtWt()
    ...
    Metric = False
    Female = False
    BMI = RxCalc_BMI(Height, Weight, Metric)
    
    ' Display calculations
    Debug.Print "BMI: " & BMI & " kg/m^2 (" & RxCalc_BMI_Class(BMI) & ")"
    Debug.Print "BSA (Du Bois): " & RxCalc_BSA_DuBois(Height, Weight, Metric) & " m^2"
    Debug.Print "BSA (Mosteller): " & RxCalc_BSA_Mosteller(Height, Weight, Metric) & " m^2"
    Debug.Print "IBW: " & RxCalc_IBW(Height, Weight, Metric) & " kg"
    Debug.Print "AdjBW: " & RxCalc_AdjBW(Height, Weight, Female, Metric) & " kg"
...
Attribute RxCalc_BMI.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    If Len(Height) = 0 Or Weight <= 0 Then
        RxCalc_BMI = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) = 0 And Len(Height) > 0 Then
        RxCalc_BMI = CVErr(xlErrDiv0)
        Exit Function
    ElseIf Val(Height) < 0 And Len(Height) > 0 Then
        RxCalc_BMI = CVErr(xlErrNum)
        Exit Function
...
Public Function RxCalc_BMI_Class(ByVal BMI As Single) As String
Attribute RxCalc_BMI_Class.VB_Description = "Classify body mass index (BMI)."
Attribute RxCalc_BMI_Class.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    Select Case BMI
        Case Is < 18.5
            RxCalc_BMI_Class = "Underweight"
        Case 18.5 To 24.9
            RxCalc_BMI_Class = "Normal"
        Case 25 To 29.9
            RxCalc_BMI_Class = "Overweight"
        Case 30 To 34.9
            RxCalc_BMI_Class = "Obese class I"
        Case 35 To 39.9
            RxCalc_BMI_Class = "Obese class II"
        Case Is >= 40
            RxCalc_BMI_Class = "Obese class III"
    End Select
...
Attribute RxCalc_BSA_DuBois.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Validate variables
    If Len(Height) = 0 Or Weight <= 0 Then
        RxCalc_BSA_DuBois = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) <= 0 And Len(Height) > 0 Then
        RxCalc_BSA_DuBois = CVErr(xlErrNum)
        Exit Function
...
Attribute RxCalc_BSA_Mosteller.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Validate variables
    If Len(Height) = 0 Or Weight <= 0 Then
        RxCalc_BSA_Mosteller = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) <= 0 And Len(Height) > 0 Then
        RxCalc_BSA_Mosteller = CVErr(xlErrNum)
        Exit Function
    ...
    ' Return BSA
    RxCalc_BSA_Mosteller = BSA

End Function

Public Function RxCalc_IBW(ByVal Height As String, _
    ByVal Female As Boolean, _
    Optional ByVal Metric As Boolean = True) As Variant
Attribute RxCalc_IBW.VB_Description = "Calculate ideal body weight (IBW) over 60 inches.\r\nFormula: IBW (Male) = 50kg + 2.3kg for each inch below 60 inches\r\nFormula: IBW (Female) = 45.5kg + 2.3kg for each inch below 60 inches\r\nOutput: IBW [kg]"
Attribute RxCalc_IBW.VB_ProcData.VB_Invoke_Func = " \n20"
...
Public Function RxCalc_AdjBW(ByVal Height As String, _
    ByVal Weight As Single, ByVal Female As Boolean, _
    Optional ByVal Metric As Boolean = True) As Variant
Attribute RxCalc_AdjBW.VB_Description = "Calculate adjusted body weight (AdjBW).\r\nFormula: AdjBW = IBW + 0.4(Actual - IBW)\r\nOutput: AdjBW [kg]"
Attribute RxCalc_AdjBW.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Calculate IBW
    IBW = RxCalc_IBW(Height, Female, Metric)
...
Public Function RxCalc_IBW_Intuitive(ByVal Height As String, _
    ByVal Female As Boolean, _
    Optional ByVal Metric As Boolean = True) As Variant
Attribute RxCalc_IBW_Intuitive.VB_Description = "Calculate ideal body weight (IBW) under 60 inches.\r\nFormula: IBW (Male) = 50kg - 2.3kg for each inch below 60 inches\r\nFormula: IBW (Female) = 45.5kg - 2.3kg for each inch below 60 inches\r\nOutput: IBW [kg].\r\nError 8888: Height over 60 inches"
Attribute RxCalc_IBW_Intuitive.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Validate variables
    If Len(Height) = 0 Then
        RxCalc_IBW_Intuitive = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) <= 0 And Len(Height) > 0 Then
        RxCalc_IBW_Intuitive = CVErr(xlErrNum)
        Exit Function
    ...
    If Val(Ht) > 60 Then
        RxCalc_IBW_Intuitive = ErrorOver60
        Exit Function
    End If
    ...
    ' Return IBW
    RxCalc_IBW_Intuitive = IBW
    
End Function

Public Function RxCalc_IBW_Baseline(ByVal Height As String, _
    ByVal Female As Boolean, _
    Optional ByVal Metric As Boolean = True) As Variant
Attribute RxCalc_IBW_Baseline.VB_Description = "Calculate ideal body weight (IBW) under 60 inches.\r\nFormula: IBW (Male) = 50kg - 0.833kg for each inch below 60 inches\r\nFormula: IBW (Female) = 45.5kg - 0.758kg for each inch below 60 inches\r\nOutput: IBW [kg].\r\nError 8888: Height over 60 inches"
Attribute RxCalc_IBW_Baseline.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Validate variables
    If Len(Height) = 0 Then
        RxCalc_IBW_Baseline = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) <= 0 And Len(Height) > 0 Then
        RxCalc_IBW_Baseline = CVErr(xlErrNum)
        Exit Function
    ...
    ' Validate variables (Must be under 60 inches)
    If Val(Ht) > 60 Then
        RxCalc_IBW_Baseline = ErrorOver60
        Exit Function
    End If
...
Attribute RxCalc_IBW_Hume.VB_ProcData.VB_Invoke_Func = " \n20"
    ...
    ' Validate variables
    If Len(Height) = 0 Or Weight <= 0 Then
        RxCalc_IBW_Hume = CVErr(xlErrNum)
        Exit Function
    ElseIf Val(Height) <= 0 And Len(Height) > 0 Then
        RxCalc_IBW_Hume = CVErr(xlErrNum)
        Exit Function
...
Public Sub RxCalc_HtWtMacroArg()
    
    Application.MacroOptions "RxCalc_BMI_Class", "Classify body mass index (BMI).", , , , , "RxCalc", , , , _
        Array("Body mass index [kg/m" & Chr(178) & "]")
    
    Application.MacroOptions "RxCalc_BMI", "Calculate body mass index (BMI)." & vbNewLine & _
        "Formula: BMI = Weight / Height" & Chr(178) & vbNewLine & _
        "Output: BMI [kg/m" & Chr(178) & "]", , , , , "RxCalc", , , , _
    ...
    Application.MacroOptions "RxCalc_BSA_DuBois", "Calculate body surface area (BSA) with Du Bois formula." & vbNewLine & _
        "Formula: BSA = 0.007184 " & Chr(215) & " Weight^0.425 " & Chr(215) & " Height^0.725" & vbNewLine & _
        "Output: BSA [m" & Chr(178) & "]", , , , , "RxCalc", , , , _
        Array("Sample formats: 5'10" & Chr(34) & " or 70 [inches or cm]", _
        "Number [lbs or kg]", _
        "OPTIONAL Specify the units of height and weight [TRUE=Metric (Default) or FALSE=US]")

    Application.MacroOptions "RxCalc_BSA_Mosteller", "Calculate body surface area (BSA) with Mosteller formula." & vbNewLine & _
    ...
    Application.MacroOptions "RxCalc_IBW", "Calculate ideal body weight (IBW) over 60 inches." & vbNewLine & _
        ...
        "Output: IBW [kg]", , , , , "RxCalc", , , , _
        Array("Sample formats: 5'10" & Chr(34) & " or 70 [inches or cm]", _
        "Boolean [TRUE=Female or FALSE=Male]", _
        "OPTIONAL Specify the units of height and weight [TRUE=Metric (Default) or FALSE=US]")

    Application.MacroOptions "RxCalc_AdjBW", "Calculate adjusted body weight (AdjBW)." & vbNewLine & _
        "Formula: AdjBW = IBW + 0.4(Actual - IBW)" & vbNewLine & _
        "Output: AdjBW [kg]", , , , , "RxCalc", , , , _
        Array("Sample formats: 5'10" & Chr(34) & " or 70 [inches or cm]", _
        "Number [lbs or kg]", _
        "Boolean [TRUE=Female or FALSE=Male]", _
        "OPTIONAL Specify the units of height and weight [TRUE=Metric (Default) or FALSE=US]")

    Application.MacroOptions "RxCalc_IBW_Intuitive", "Calculate ideal body weight (IBW) under 60 inches." & vbNewLine & _
        "Formula: IBW (Male) = 50kg - 2.3kg for each inch below 60 inches" & vbNewLine & _
        "Formula: IBW (Female) = 45.5kg - 2.3kg for each inch below 60 inches" & vbNewLine & _
        "Output: IBW [kg]." & vbNewLine & "Error " & ErrorOver60 & ": Height over 60 inches", , , , , "RxCalc", , , , _
        Array("Sample formats: 5'10" & Chr(34) & " or 70 [inches or cm]", _
        "Boolean [TRUE=Female or FALSE=Male]", _
        "OPTIONAL Specify the units of height and weight [TRUE=Metric (Default) or FALSE=US]")

    Application.MacroOptions "RxCalc_IBW_Baseline", "Calculate ideal body weight (IBW) under 60 inches." & vbNewLine & _
    ...
    Application.MacroOptions "RxCalc_IBW_Hume", "Calculate ideal body weight (IBW) under 60 inches." & vbNewLine & _
